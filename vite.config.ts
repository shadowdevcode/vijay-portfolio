            ...
            req.on('end', async () => {
              try {
                ...
                if (!message || typeof message !== 'string' || !message.trim()) {
                  res.statusCode = 400;
                  res.setHeader('Content-Type', 'application/json');
                  res.end(JSON.stringify({ error: { code: 'INVALID_INPUT', message: 'A non-empty "message" is required.' } }));
                  return;
                }
                ...
                if (!apiKey) {
                  res.statusCode = 500;
                  res.setHeader('Content-Type', 'application/json');
                  res.end(JSON.stringify({ error: { code: 'CONFIG_ERROR', message: 'GEMINI_API_KEY not set in .env' } }));
                  return;
                }
                ...
                console.error('[Dev API] Gemini error:', error?.message || error);
                res.statusCode = 500;
                res.setHeader('Content-Type', 'application/json');
                res.end(JSON.stringify({ error: { code: 'DEV_ERROR', message: 'Chat error in dev mode. Check terminal.' } }));
              }
            });
          });
        },
