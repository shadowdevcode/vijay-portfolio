 ...
 *
 * The system prompt is imported from api/system-prompt.ts so
 * dev (vite.config.ts) and prod share a single source of truth.
 *
...
import { CHAT_MAX_MESSAGE_LENGTH, RATE_LIMIT_WINDOW_MS, RATE_LIMIT_MAX } from '../config';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (errStr.includes('403') || errStr.includes('API_KEY')) {
